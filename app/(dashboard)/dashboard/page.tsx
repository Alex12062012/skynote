...
import { 
  User, Mail, Coins, Flame, Target, Trophy, BookOpen, 
...
export default async function ProfilePage() {
  ...
  const coinsNeeded = 100 - coins
  const showPremiumBanner = !isPremium && coins >= 60 && coins < 100

  // Stats mockées (à remplacer par de vraies requêtes si besoin)
  const stats = {
    courses: 0,
    qcmDone: 0,
    perfectScores: 0,
  ...
  return (
    <div className="flex flex-col gap-8 animate-fade-in max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-4 rounded-card border border-brand/20 bg-brand-soft p-4 dark:border-brand-dark/20 dark:bg-brand-dark-soft">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-body text-[14px] font-semibold text-brand dark:text-brand-dark">
              Plus que {coinsNeeded} coins pour débloquer Premium ! ⭐
            </p>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {[
          { label: 'Cours', value: stats.courses, icon: <BookOpen className="h-5 w-5 text-sky-500" /> },
          { label: 'QCM faits', value: stats.qcmDone, icon: <Zap className="h-5 w-5 text-amber-500" /> },
          { label: 'Scores parfaits', value: stats.perfectScores, icon: <Trophy className="h-5 w-5 text-emerald-500" /> },
        ].map((s) => (
          <div 
            key={s.label} 
      ...
      <div className="flex items-center gap-3 rounded-card border border-orange-200 bg-orange-50 p-4 dark:border-orange-900/30 dark:bg-orange-900/10">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-orange-700/80 dark:text-orange-400/80 mt-0.5">
            Connecte-toi chaque jour pour garder ta série !
          </p>
        </div>
      </div>
