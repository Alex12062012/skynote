      ...
      <div className="flex min-h-screen items-center justify-center bg-[#060D1A] px-4">
        <div className="w-full max-w-sm">
          <div className="mb-8 text-center">
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-blue-600 text-3xl">📊</div>
      ...
      <div ref={printRef} className="print-page max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="flex items-start justify-between mb-10">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-blue-600 text-2xl">☁️</div>
              <div>
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              { label: 'Utilisateurs inscrits', value: k.totalUsers, icon: '👥', color: 'text-blue-400' },
              { label: 'Actifs aujourd\'hui', value: k.dau, sub: `${k.dauRate}% DAU`, icon: '⚡', color: 'text-emerald-400' },
              { label: 'Actifs cette semaine', value: k.wau, sub: `${k.wauRate}% WAU`, icon: '📅', color: 'text-purple-400' },
              { label: 'Rétention J+7', value: `${k.retention7d}%`, icon: '🔁', color: 'text-amber-400' },
            ].map(item => (
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              { label: 'Cours créés', value: k.totalCourses, icon: '📚' },
              { label: 'QCM faits', value: k.totalQcm, icon: '⚡' },
              { label: 'Moy. cours / user', value: k.avgCoursesPerUser, icon: '📖' },
              { label: 'Moy. QCM / user', value: k.avgQcmPerUser, icon: '🎯' },
              { label: 'Taux de réussite', value: `${k.perfectRate}%`, icon: '🏆' },
              { label: 'Streak moyen', value: `${k.avgStreak}j`, icon: '🔥' },
              { label: 'Fiches générées', value: k.totalFlashcards, icon: '📝' },
              { label: 'Sky Coins distribués', value: k.totalCoins.toLocaleString('fr-FR'), icon: '🪙' },
            ].map(item => (
              <div key={item.label} className="card rounded-xl border border-slate-800 bg-slate-900 p-4">
