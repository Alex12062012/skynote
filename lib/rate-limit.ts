...
export async function checkRateLimit(
  ...
  /** Unités consommées par cet appel (ex. 1 par fiche pour la route par niveau). */
  amount = 1
...
export const RATE_LIMITS = {
  ...
  // Une unité = un appel Claude (une fiche × un niveau) : 40/h = 2 cours de
  // 6 fiches par heure. NB : la migration 20260610_rate_limits n'avait jamais
  // ete appliquee en prod avant le 2026-09-16 — aucun plafond n'etait actif.
 ...
 */
export async function checkQcmRateLimits(userId: string, amount = 1): Promise<Response | null> {
  for (const [key, cfg, libelle] of [
    ...
    const rl = await checkRateLimit(userId, key, cfg, amount)
    if (!rl.allowed) {
