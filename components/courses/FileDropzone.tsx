    ...
    <div>
      ...
      {currentFile ? (
        <div className="flex items-center justify-between rounded-input border border-brand/30 bg-brand-soft px-4 py-3 dark:border-brand-dark/30 dark:bg-brand-dark-soft">
          <span className="font-body text-[13px] text-brand truncate dark:text-brand-dark">{currentFile.name}</span>
          <button type="button" onClick={() => handleFile(null)} className="ml-2 flex-shrink-0 text-text-tertiary hover:text-text-main dark:hover:text-text-dark-main">
            <X className="h-4 w-4" />
          </button>
        </div>
      ) : (
        <div
          ...
          <div className="text-center">
            ...
            <p className="font-body text-[12px] text-text-tertiary dark:text-text-dark-tertiary mt-1">ou glisse-depose ici</p>
          </div>
        </div>
      )}
