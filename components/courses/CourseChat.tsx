...
export function CourseChat({ courseId, courseTitle, isPremium }: CourseChatProps) {
  ...
  // Version verrouillée (non premium)
  if (!isPremium) {
    return (
      <div className="flex items-center gap-3 rounded-card border border-sky-border bg-sky-surface p-4 dark:border-night-border dark:bg-night-surface">
        ...
        <a
          href="/pricing"
          className="flex-shrink-0 rounded-input bg-brand px-3 py-1.5 font-body text-[12px] font-semibold text-white hover:bg-brand-hover dark:bg-brand-dark dark:text-night-bg transition-colors"
        >
          Voir les plans
        </a>
      </div>
        ...
        <div className="rounded-card border border-brand/30 bg-sky-surface overflow-hidden dark:border-brand-dark/30 dark:bg-night-surface">
          ...
          <div className="flex items-center justify-between border-b border-sky-border px-4 py-3 dark:border-night-border">
            <div className="flex items-center gap-2">
              ...
              <span className="font-body text-[14px] font-semibold text-text-main dark:text-text-dark-main truncate">
                Chatbot — {courseTitle}
              </span>
            </div>
            <button
              onClick={() => setOpen(false)}
              className="flex-shrink-0 text-text-tertiary hover:text-text-main dark:hover:text-text-dark-main transition-colors"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
          ...
          <div
            ref={scrollRef}
            className="flex flex-col gap-3 p-4 max-h-80 overflow-y-auto min-h-[120px]"
          >
            {messages.length === 0 && (
              ...
              <div className="flex justify-start">
                <div className="flex items-center gap-1.5 rounded-2xl bg-sky-cloud px-4 py-3 dark:bg-night-border rounded-bl-md">
                  {[0, 1, 2].map((i) => (
                    <div
                      key={i}
                      className="h-1.5 w-1.5 rounded-full bg-text-tertiary animate-bounce dark:bg-text-dark-tertiary"
