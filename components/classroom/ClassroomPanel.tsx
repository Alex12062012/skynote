...
export function ClassroomPanel({ classCode, students, siteUrl }: ClassroomPanelProps) {
  ...
  const loginUrl = `${siteUrl}/demo-login`
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="rounded-card bg-brand-soft/30 border border-brand/10 p-5 dark:bg-brand-dark-soft/30 dark:border-brand-dark/10">
        <h3 className="font-display text-[15px] font-semibold text-brand dark:text-brand-dark mb-2">
          Comment les élèves accèdent à la classe
        </h3>
        <div className="space-y-2 font-body text-[13px] text-text-secondary dark:text-text-dark-secondary">
          <p>1. L&apos;élève va sur <span className="font-medium text-brand dark:text-brand-dark">{loginUrl}</span></p>
          <p>2. Il tape son code personnel (ex: aroudaut{classCode})</p>
          <p>3. Il accède directement aux cours et QCM de la classe</p>
        </div>
        <button
        ...
        >
          ...
          {copied === loginUrl ? 'Copié !' : 'Copier le lien de connexion'}
        </button>
      </div>
      ...
      <div className="rounded-card bg-sky-surface p-6 shadow-card dark:bg-night-surface">
        ...
        <div className="flex items-center justify-between px-4 py-2 mb-1">
          <span className="font-body text-[11px] font-medium uppercase tracking-wider text-text-tertiary dark:text-text-dark-tertiary">Élève</span>
        ...
        <div className="space-y-1.5 max-h-[400px] overflow-y-auto">
          {students.map((s, i) => (
            <div key={i} className="flex items-center justify-between rounded-input bg-sky-bg px-4 py-3 dark:bg-night-bg">
              <div className="flex items-center gap-3">
