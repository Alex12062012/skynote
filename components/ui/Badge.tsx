...
const SUBJECT_COLORS: Record<string, { bg: string; text: string; label?: string }> = {
  'Mathematiques':      { bg: 'bg-blue-100 dark:bg-blue-950/30',   text: 'text-blue-700 dark:text-blue-400',   label: 'Mathematiques' },
  'Francais':           { bg: 'bg-pink-100 dark:bg-pink-950/30',   text: 'text-pink-700 dark:text-pink-400',   label: 'Francais' },
  'Histoire-Geographie':{ bg: 'bg-amber-100 dark:bg-amber-950/30', text: 'text-amber-700 dark:text-amber-400', label: 'Histoire-Geo' },
  ...
  'General':            { bg: 'bg-sky-100 dark:bg-sky-950/30',     text: 'text-sky-700 dark:text-sky-400',     label: 'General' },
  'Histoire':           { bg: 'bg-amber-100 dark:bg-amber-950/30', text: 'text-amber-700 dark:text-amber-400' },
  'Geographie':         { bg: 'bg-emerald-100 dark:bg-emerald-950/30', text: 'text-emerald-700 dark:text-emerald-400' },
}
...
export function PlanBadge({ plan }: { plan: string }) {
  ...
  return (
    <a href="/pricing" className="inline-flex items-center rounded-pill bg-sky-cloud px-3 py-1 font-body text-[12px] font-medium text-text-secondary hover:bg-sky-border transition-colors dark:bg-night-border dark:text-text-dark-secondary dark:hover:bg-sky-border/20 cursor-pointer">
      Gratuit
    </a>
  )
}
